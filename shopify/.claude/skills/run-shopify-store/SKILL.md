...
- **`productSet` silently drops variant prices.** Creating a product with
  `variants: [{ price: "62.00", ... }]` produces variants at **$0.00** with no
  userError. Set prices in a follow-up `productVariantsBulkUpdate` and verify —
  a rebuilt product will otherwise go live free.
