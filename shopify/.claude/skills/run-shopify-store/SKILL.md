...
- **Inventory writes are fiddly (API 2026-04).** Use `inventoryAdjustQuantities` (deltas) or `inventorySetQuantities`, and:
  ...
  - A **bundle's** available qty is its lowest-stock component, so to make a box orderable you stock its *components*, not the box (Welcome's 11 components were set to 30 this way). Location: `INSTAMOM U` = `gid://shopify/Location/90382762084`.
...
- **`Access denied for locations field` / `publications field` / `read_publications access scope`**: your cached auth lacks that scope. Re-run `shopify store auth …` with the scope added (see Gotchas).
